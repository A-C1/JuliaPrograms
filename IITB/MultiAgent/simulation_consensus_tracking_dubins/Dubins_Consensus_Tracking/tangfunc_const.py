...
def tang_dist(ta, pflag, xp_cen, yp_cen, rp, vp, xp, yp, eflag, xe_cen, ye_cen,
    ...
    # t_end: time after which the pursuer and evader both
    # will be on the common tangent.
    t_end = np.max([2*np.pi*re/ve, 2*np.pi*rp/vp])
    # Remove the time for which the evader and pursuer are
    # on their circles from t_end
    t_rem_e = t_end - dist_e/ve
    t_rem_p = t_end - dist_p/vp
    # Finally compute the distance between pursuer and evader
    # at end of t_end
