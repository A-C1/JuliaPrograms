...
# tangfunc(x_A, y_A, r_A, x_B, y_B, r_B)
def calccenters(vp, up, z0p, ve, ue, z0e):
    # Fixing all the four circles and drawing them
    # Anti Clockwise Circle of Pursuer
    xpa = z0p[0] - (vp/up)*np.sin(z0p[2])
    ypa = z0p[1] + (vp/up)*np.cos(z0p[2])
    rp = vp/up

    # Clockwise Circle of Pursuer
    xpc = z0p[0] + (vp/up)*np.sin(z0p[2])
    ypc = z0p[1] - (vp/up)*np.cos(z0p[2])

    # Anti Clockwise Circle of Evader
    xea = z0e[0] - (ve/ue)*np.sin(z0e[2])
    yea = z0e[1] + (ve/ue)*np.cos(z0e[2])
    re = ve/ue

    # Clockwise Circle of Evader
    xec = z0e[0] + (ve/ue)*np.sin(z0e[2])
    yec = z0e[1] - (ve/ue)*np.cos(z0e[2])
...
#
    ...
    dist_p = rp*angle_p*np.pi/180
    dist_e = re*angle_e*np.pi/180
