...
for i in range(0, no_iter - 1):
    ...
    dist1_c = np.linalg.norm(v12-states_e[i+1,:])
    dist2_c = np.linalg.norm(v13-states_e[i+1,:])
    dist3_c = np.linalg.norm(v23-states_e[i+1,:])
    dist4_c = np.linalg.norm(v4-states_e[i+1,:])
