...
def time_opt(sys_dyn, sys_der, sys_dim, num_inputs, psi, psi_der, dim_psi,
             phi, phi_der, L, L_der, x_initial, x_final, state_limits_L,
             state_limits_U, time_limit_L, time_limit_U, N,
             extra_data):
    ...
    x_L = np.ones((nvar)) * 0  # Lower limits of Varaibles
    x_U = np.ones((nvar)) * 0  # Upper Limits of Variables

    for i in range(0, N):
        for j in range(1, new_dim+1):
            x_L[i*new_dim + j] = state_limits_L[j-1]   # Assigning lower limits
            x_U[i*new_dim + j] = state_limits_U[j-1]   # Assigning upper limits

    # Time is always positive
    x_L[0] = time_limit_L  # T_lower is set to a very small positive value
    x_U[0] = time_limit_U  # T_upper is set equal to 50
    ...
    for i in range(0, N):
        ...
        for j in range(0, num_inputs):
            u[j][i] = x0[1 + sys_dim + j + i*new_dim]
        for j in range(0, sys_dim):
            y[j][i] = x0[(1+j+i*new_dim)]
    ...
    return constraint_multipliers, y, u, t, obj, status
