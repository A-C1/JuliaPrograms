...
x_initial_p = [0, 0, 5*np.pi/6]  # Initial Conditions
x_final_p = ["rel", "rel", "rel"]  # Final State,"rel" if free
# First Write the limits of states and then that of inputs
# [x' u']
state_limits_L_p = [-10, -10, -6.4, -0.6]  # Lower State limits
state_limits_U_p = [20, 20, 6.4, 0.6]
time_limit_L_p = 0.1/N_p
time_limit_U_p = 20/N_p
...
x_initial_e = np.array([1, 1, -np.pi*0])  # Initial Conditions
x_final_e = ["rel", "rel", "rel"]  # Final State,"rel" if free
# First Write the limits of states and then that of inputs
# [x' u']
state_limits_L_e = [-2, -2, 0, -0.3]  # Lower State limits
state_limits_U_e = [20, 20, 6.4, 0.3]
...
for i in range(0, 1):
    ...
    # lambda_l = lambdas[-4:-1]     # Value of costate of pursuer at time of capture
